...
Raspberry Pi 4B에서 TensorFlow Lite 객체 검출 모델과 직접 구현한 Linux 문자 디바이스
드라이버를 결합하여, 영상에 포착된 대상 객체를 실시간으로 계수하고 그 수가 사용자가 버튼으로
지정한 기준값을 넘으면 GPIO에 연결된 LED 등의 주변장치를 구동하여 경보 동작을 수행한다.

본 프로젝트의 목적은 객체 검출 자체가 아니라, **사용자 공간의 추론 결과가
...
> **검출 대상 클래스에 관하여**
>
> 제출 구현은 검출 대상을 COCO 라벨맵의 `person` 클래스 하나로 한정하였다. 이는
> `ProjectCode/main.cpp`의 주석("그러나 person에 대해서만 한정해서 생각")에 명시되어 있다.
> 사용 중인 SSD MobileNet 모델은 COCO 90개 클래스를 모두 예측하므로 검출 루프의 클래스
> 비교 한 줄을 교체하면 대상을 바꿀 수 있으며, 전처리·계수·디바이스 제어에 이르는 나머지
> 경로는 검출 클래스에 의존하지 않는다.
...
flowchart TB
    ...
    subgraph user["사용자 공간 · TermProject17"]
        ...
        inf["TFLite · SSD MobileNet<br>대상 클래스 검출 계수 → N"]
        cap --> inf
    end
...
4. 대상 클래스에 해당하는 검출만을 계수하여 현재 개체 수 **N**을 구한다
   (제출 구현의 대상 클래스는 `person`).
5. `/dev/my_button`을 읽어 버튼 입력에 따라 경보 기준값 **M**을 증감한다 (0 – 9999 순환).
6. `/dev/my_segment`에 **M**을 기록하여 7-세그먼트에 표시한다.
7. **N ≥ M**이면 `/dev/my_led`에 `'u'`를, 아니면 `'d'`를 기록하여 경보 LED를 제어한다.
...
| 경보 LED | `/dev/my_led` | 22 | 출력 | `unsigned char` | `'u'` 점등, `'d'` 소등 |
...
초기 설계는 졸음 감지를 목표로 하였다. dlib의 68점 얼굴 랜드마크 검출기로 눈 영역을 추출한
뒤, 26 × 34 흑백 영상을 입력받아 눈의 개폐 여부를 판별하는 이진 분류 CNN을 연결하는
구성이다.

이 접근은 외부에 공개된 PyTorch 구현을 참고하여 시작하였다. 참고 구현은 한때
`ProjectCode/sleep_detect/`에 포함되어 있었으며(2023-12-07 도입, 12-17 제거),
`EyeStateClassifier/`는 이를 TensorFlow Lite로 변환하기 위해 Keras로 재구현한 것이다.
입력 규격(26 × 34 흑백)이 참고 구현과 동일하다.

> 참고 구현 출처: ys-cs17, *Sleepy eyes detector* — <https://ys-cs17.tistory.com/24>
...
1. 얼굴 랜드마크 검출기(약 69 MB)와 분류망을 함께 적재할 경우 보드의 실시간 처리 성능이
   확보되지 않았다.
2. 재구현한 분류망의 일부 연산이 TFLite 변환 과정에서 호환되지 않아 추가 수정이 필요하였다
   (`model.py` 주석 참조).
...
판별"에서 "검출된 개체 수의 기준 초과 여부 판정 및 경보"로 재정의하였다. 이로써 학습 부담을 제거하고 커널
드라이버와 사용자 공간 응용 간의 연동이라는 본 과목의 핵심 주제에 집중할 수 있었다. 검출
대상을 라벨맵 수준에서 교체할 수 있게 된 것도 이 선택의 이점이다.
...
따라서 화면에 대상 객체가 4개 이상 존재하더라도 계수는 3을 초과하지 않으며,
경보 기준값을 4 이상으로 설정하면 경보가 발생하지 않는다.
