...
농장 등에서의 활용을 가정한 데모 시스템이다. Raspberry Pi 4B에서 TensorFlow Lite 객체 검출
모델과 직접 구현한 Linux 문자 디바이스 드라이버를 결합하여, 영상에 포착된 대상 객체를
실시간으로 계수하고 그 수가 사용자가 지정한 기준을 넘으면 GPIO에 연결된 LED 등의 주변장치를
구동하여 경보 동작을 수행한다. 감시 구역에서 동물이 감지되는 상황을 알리는 용도를 상정하였다.
...
> **검출 대상 클래스에 관하여**
>
> 제출 구현은 시연 편의를 위해 검출 대상을 COCO 라벨맵의 `person` 클래스로 한정하였으며,
> 이는 `ProjectCode/main.cpp`의 주석에 명시되어 있다. 사용 중인 SSD MobileNet 모델의
> 라벨맵에는 `bird`, `cat`, `dog`, `horse`, `sheep`, `cow` 등 동물 클래스가 이미 포함되어
> 있으므로, 검출 루프의 클래스 비교 한 줄을 교체하면 대상을 전환할 수 있다. 전처리, 계수,
> 디바이스 제어에 이르는 나머지 경로는 검출 클래스에 의존하지 않는다.
...
### 7.1 1차 접근 — 눈 상태 분류기 (폐기)

초기 설계에서는 dlib의 68점 얼굴 랜드마크 검출기로 눈 영역을 추출한 뒤, 26 × 34 흑백
영상을 입력받아 눈의 개폐 여부를 판별하는 이진 분류 CNN을 직접 학습시키는 구성을 시도하였다. 해당 파이프라인은 `EyeStateClassifier/`에 남아 있다.
...
자체 학습망 대신 COCO로 사전 학습된 SSD MobileNet 양자화 모델을 채택하고, 문제를 "검출된
개체 수의 기준 초과 여부 판정 및 경보"로 재정의하였다. 이로써 학습 부담을 제거하고 커널
