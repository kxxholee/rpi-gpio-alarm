    ...
    1. 예제 구현 내용
        ...
        - B. Data
            ...
            - `# sudo insmod seg_driver.ko`
                ...
                <img src="./EMSYS_WEEK10_SUBDOC/seg_driver_insmod.png" alt="Insmod" width="640" height="360"/>
    ...
    2. 과제 구현 내용
            ...
            - **segButton.c**
                ...
                int main(int argc, char* argv[]) {      /** MAIN: */
                    ...
                    for (char command = -1; command != 'q'; get_key(&command)) {
                        unsigned char  ButtonInput = '0'; // -> 2 * sizeof(char) == sizeof(short)
                        ...
                        if      (ButtonInput == 'u') SegmentDisplay = (SegmentDisplay >= 9999) ? 0 : SegmentDisplay + 1;
                        else if (ButtonInput == 'd') SegmentDisplay = (SegmentDisplay <= 0) ? 9999 : SegmentDisplay - 1;
                        ...
                        for (unsigned char index = 0; index < 4; index++) {
                            write(Segment, &DisplayBuff[index], sizeof(unsigned short));
                            usleep(SEGMENT_DELAY);
                        }
                        usleep(LOOP_DELAY);
                    }

                    write(Segment, (char*)NULL, sizeof(char) * 3);
                    close_keyboard();
                    close(Segment);
                    close(Button);
        ...
        - B. Data
            - `# make`
                ...
                <img src="./EMSYS_WEEK10_SUBDOC/HW-make.png" alt="make result" width="640" height="360"/>

            - `# sudo insmod seg_driver.ko && sudo insmod button_driver.ko`

                <img src="./EMSYS_WEEK10_SUBDOC/HWdmesg.png" alt="insmod and dmesg" width="640" height="360"/>
        ...
        - C. Discussion
            ...
            - `seg_driver.c` 
                커널의 메세지 부분에만 사소한 수정이 있었고, 이외의 부분은 크게 수정하지 않았다.
            - `button_driver.c`
                버튼에서 오는 입력을, UP버튼인지 DOWN버튼인지에 따라 각기 다른 버퍼에 담아 자료를 이동시키도록 작성했다. 따라서 소스 코드에서는 `read()` 함수를 통해 버튼의 값을 읽을 수 있다.
            - `segButton.c`
                드라이버에서 제공하는 값을 읽어 오고, 이에 따라 7-Segment Device에 표시할 숫자를 증가/감소시키도록 작성하였다. `write()`함수를 올바르게 사용해 이 숫자를 7-Segment Device에 표시할 수 있었다.
